...
class AnalysisPlots(object):
    ...
    def __init__(self,X,Y,D1,D2,nfeatures,A,B, manip_mode='TOCSY', dbk=None, cmap=None, title="my name", levels = [0.5,1,2,5,10,20,50,100],mode="RFE"):
        ...
        self.levels = levels  
        if self.mode == "RFE": 
            xs,ys,col = get_contour_data(affiche(*(RecurFeatElim(self.X, ty(self.Y,self.A_slider_value,self.B_slider_value), self.D1, self.D2,nfeatures=self.slider_value)), cmap=self.colormap,levelbase=self.levels))
            self.source = ColumnDataSource(data=dict(xs=xs, ys=ys, color=col))
            ...
            xs,ys,col = get_contour_data(affiche(*(LinRegression(self.X, ty(self.Y,self.A_slider_value,self.B_slider_value), self.D1, self.D2, nfeatures=self.slider_value)), cmap=self.colormap,levelbase=self.levels))
            self.source = ColumnDataSource(data=dict(xs=xs, ys=ys, color=col))
            line_x = ty(self.x,self.A_slider_value,self.B_slider_value)
            line_y = self.x
            points_x = ty(self.Y,self.A_slider_value,self.B_slider_value) #Corrected Y
            ...
            xs,ys,col = get_contour_data(affiche(*(LogisticRegr(self.X, ty(self.Y,self.A_slider_value,self.B_slider_value), self.D1, self.D2, nfeatures=self.slider_value)), cmap=self.colormap,levelbase=self.levels))
            self.source = ColumnDataSource(data=dict(xs=xs, ys=ys, color=col))
            line_x = ty(self.x,self.A_slider_value,self.B_slider_value)
            line_y = self.x
            points_x = ty(self.Y,self.A_slider_value,self.B_slider_value) #Corrected Y
    ...
    def update_data(self,attr, old, new):
        # Get the current slider value & update source
        if self.mode == "RFE": 
            xs,ys,col = get_contour_data(affiche(*(RecurFeatElim(self.X, ty(self.Y,self.A_slider.value,self.B_slider.value), self.D1, self.D2, nfeatures=self.nfeatures_slider.value)), cmap=self.colormap,levelbase=self.levels))
            ...
            xs,ys,col = get_contour_data(affiche(*(LinRegression(self.X, ty(self.Y,self.A_slider.value,self.B_slider.value), self.D1, self.D2, nfeatures=self.nfeatures_slider.value)), cmap=self.colormap,levelbase=self.levels))
            self.source.data = dict(xs=xs, ys=ys, color=col)
            line_x = ty(self.x,self.A_slider.value,self.B_slider.value)
            line_y = self.x
            points_x = ty(self.Y,self.A_slider.value,self.B_slider.value) #Corrected Y
            ...
            xs,ys,col = get_contour_data(affiche(*(LogisticRegr(self.X, ty(self.Y,self.A_slider.value,self.B_slider.value), self.D1, self.D2, nfeatures=self.nfeatures_slider.value)), cmap=self.colormap,levelbase=self.levels))
            self.source.data = dict(xs=xs, ys=ys, color=col)
            line_x = ty(self.x,self.A_slider.value,self.B_slider.value)
            line_y = self.x
            points_x = ty(self.Y,self.A_slider.value,self.B_slider.value) #Corrected Y
